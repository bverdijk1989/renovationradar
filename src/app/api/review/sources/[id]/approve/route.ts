...
import { approveSourceReview } from "@/server/services/review";
...
export const POST = withApi<Params>(async (req: NextRequest, ctx) => {
  ...
  const source = await approveSourceReview(id, admin.id, body.notes);
  await auditLog({
    req,
    userId: admin.id,
    action: "source_check",
    entityType: "source",
    entityId: id,
    meta: { decision: "approve", legalStatus: source.legalStatus, status: source.status },
  });
  return ok(source);
});
